...
* For this, we need to keep track of the end time of the meeting happening in it. If the end time of m2 is before the
* start time of m4, we can use that room r1, otherwise, we need to schedule m4 in another room r3.
We can conclude that we need to keep track of the ending time of all the meetings currently happening so that when we
* try to schedule a new meeting, we can see what meetings have already ended. We need to put this information in a
* data structure that can easily give us the smallest ending time. A Min Heap would fit our requirements best.

So our algorithm will look like this:

Sort all the meetings on their start time.
Create a min-heap to store all the active meetings. This min-heap will also be used to find the active meeting with the
* smallest end time.
...
The heap will always have all the overlapping meetings, so we will need rooms for all of them. Keep a counter to
* remember the maximum size of the heap at any time which will be the minimum number of rooms needed.*/
import java.util.*;
...
public class MinimumMeetingRooms {
    ...
    public static int findMinimumMeetingRooms(List<Meeting> meetings){
        ...
        //sort all meetings based on their start time.
        Collections.sort(meetings, (a,b) -> Integer.compare(a.start, b.start));

        int minRooms = 0;
        PriorityQueue<Meeting> minHeaps = new PriorityQueue<>(meetings.size(), (a, b) -> Integer.compare(a.end, b.end));

        for (Meeting meeting : meetings){

            //remove all meetings that have ended
            while (!minHeaps.isEmpty() && meeting.start >= minHeaps.peek().end){
                minHeaps.poll();
            }

            //add the active meeting to heap
            minHeaps.offer(meeting);

            // all active meeting are in the minHeap, so we need rooms for all of them.
            minRooms = Math.max(minRooms, minHeaps.size());
        }
        return minRooms;
    }
